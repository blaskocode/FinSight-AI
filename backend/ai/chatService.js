 ...
 */
function getOrCreateSession(userId, conversationId) {
 ...
 */
async function createSystemPrompt(userId) {
    // Get user info
 ...
 */
async function queryTransactions(userId, category, dateRange, merchantName) {
    ...
    const userTransactionCount = await (0, db_1.all)(`SELECT COUNT(*) as count 
     FROM transactions t
     INNER JOIN accounts a ON t.account_id = a.account_id
     WHERE a.user_id = ?`, [userId]);
    console.log('Total transactions for user:', userTransactionCount[0]?.count || 0);
    // Check transactions in the date range
    const dateRangeCount = await (0, db_1.all)(`SELECT COUNT(*) as count 
     FROM transactions t
     INNER JOIN accounts a ON t.account_id = a.account_id
     WHERE a.user_id = ? AND t.date >= ? AND t.date <= ?`, [userId, cutoffDate, endDate]);
    ...
    if (category) {
        const categoryCount = await (0, db_1.all)(`SELECT COUNT(*) as count 
       FROM transactions t
       INNER JOIN accounts a ON t.account_id = a.account_id
       WHERE a.user_id = ? 
         AND t.date >= ? 
         AND t.date <= ?
         AND (UPPER(t.personal_finance_category_primary) LIKE UPPER(?) OR UPPER(t.personal_finance_category_detailed) LIKE UPPER(?))`, [userId, cutoffDate, endDate, `%${category}%`, `%${category}%`]);
    ...
    const existingCategories = await (0, db_1.all)(`SELECT DISTINCT t.personal_finance_category_primary, t.personal_finance_category_detailed, COUNT(*) as count
     FROM transactions t
     INNER JOIN accounts a ON t.account_id = a.account_id
     WHERE a.user_id = ? AND t.date >= ? AND t.date <= ?
     GROUP BY t.personal_finance_category_primary, t.personal_finance_category_detailed
     ORDER BY count DESC
     LIMIT 10`, [userId, cutoffDate, endDate]);
    ...
    WHERE a.user_id = ?
      ...
      AND t.date <= ?
  `;
    const params = [userId, cutoffDate, endDate];
    if (category) {
    ...
    const transactions = await (0, db_1.all)(query, params);
 ...
 */
async function processChatMessage(userId, message, conversationId) {
    ...
    const normalizedQuery = (0, cache_1.normalizeQuery)(message);
    ...
    if (!isTransactionQuery && (session.messages.length === 0 || session.messages.length === 1)) {
        const cacheKey = (0, cache_1.generateCacheKey)(userId, normalizedQuery);
        const cachedResponse = await (0, cache_1.getCachedResponse)(cacheKey);
        if (cachedResponse) {
            console.log('[CACHE] Returning cached response for non-transaction query');
            (0, cache_1.logCacheOperation)(userId, true);
    ...
    try {
        ...
        if (assistantMessage.function_call) {
            ...
            if (functionName === 'queryTransactions') {
                const transactions = await queryTransactions(userId, functionArgs.category, functionArgs.dateRange, functionArgs.merchantName);
                functionResult = {
                    transactions,
            ...
            if (!isTransactionQuery) {
                const cacheKey = (0, cache_1.generateCacheKey)(userId, normalizedQuery);
                await (0, cache_1.cacheResponse)(cacheKey, userId, response, 1);
            }
            (0, cache_1.logCacheOperation)(userId, false);
