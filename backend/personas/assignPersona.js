...
exports.assignHighUtilizationPersona = assignHighUtilizationPersona;
exports.storePersonaAssignment = storePersonaAssignment;
exports.getCurrentPersona = getCurrentPersona;
exports.assignVariableIncomePersona = assignVariableIncomePersona;
exports.assignSubscriptionHeavyPersona = assignSubscriptionHeavyPersona;
exports.assignSavingsBuilderPersona = assignSavingsBuilderPersona;
exports.assignLifestyleCreepPersona = assignLifestyleCreepPersona;
exports.assignPersona = assignPersona;
 ...
 */
async function assignHighUtilizationPersona(userId) {
    // Get user's credit card accounts
    const creditAccounts = await (0, db_1.all)(`SELECT account_id FROM accounts 
     WHERE user_id = ? AND type = 'credit'`, [userId]);
    ...
    const primaryAccountSignals = await (0, creditMonitoring_1.getCreditSignals)(highestUtilAccount.account_id, 90);
    primarySignals = {
 ...
 */
async function storePersonaAssignment(userId, assignment, secondaryPersonas = []) {
    const personaId = `persona-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    await (0, db_1.run)(`INSERT INTO personas (persona_id, user_id, persona_type, window_days, signals, secondary_personas)
     VALUES (?, ?, ?, ?, ?, ?)`, [
        personaId,
        userId,
...
async function getCurrentPersona(userId) {
    const persona = await (0, db_1.get)(`SELECT persona_id, persona_type, assigned_at, signals, secondary_personas
     FROM personas 
     WHERE user_id = ? 
 ...
 * @returns Persona assignment result or null if criteria not met
 */
async function assignVariableIncomePersona(userId) {
 ...
 * @returns Persona assignment result or null if criteria not met
 */
async function assignSubscriptionHeavyPersona(userId) {
 ...
 * @returns Persona assignment result or null if criteria not met
 */
async function assignSavingsBuilderPersona(userId) {
    const savingsAnalysis = await (0, savingsAnalysis_1.getSavingsAnalysis)(userId, 90);
    ...
    if (meetsSavingsCriteria && allUtilizationsLow) {
        ...
        let confidence = 0.7;
        if (hasGrowth && savingsAnalysis.savingsGrowthRate > 5)
            confidence += 0.1;
        if (hasHighInflow && monthlyInflow > 300)
            confidence += 0.1;
        return {
 ...
 * @returns Persona assignment result or null if criteria not met
 */
async function assignLifestyleCreepPersona(userId) {
    ...
    const allUsers = await (0, db_1.all)('SELECT user_id FROM users');
    const allIncomes = [];
    for (const user of allUsers) {
        const userIncomeAnalysis = await (0, incomeStability_1.getIncomeStabilityAnalysis)(user.user_id);
    ...
    const discretionaryTransactions = await (0, db_1.all)(`SELECT ABS(t.amount) as amount
     FROM transactions t
    ...
    if (isHighIncome && hasLowSavingsRate && hasHighDiscretionary) {
        ...
        // Confidence based on how extreme the conditions are
        let confidence = 0.7;
        if (monthlyIncome > incomeThreshold * 1.5)
            confidence += 0.1; // Very high income
        if (savingsRate < 2)
            confidence += 0.1; // Very low savings
        if (discretionaryShare > 40)
            confidence += 0.1; // Very high discretionary
        return {
 ...
 */
async function calculateComprehensiveMetrics(userId) {
    const metrics = {};
    ...
    if (creditAccounts.length > 0) {
        const creditSignals = await (0, creditMonitoring_1.getCreditSignals)(creditAccounts[0].account_id, 90);
        metrics.utilization = creditSignals.utilization;
        metrics.interest_charges = creditSignals.interestCharges;
    }
    // Savings metrics
    const savingsAnalysis = await (0, savingsAnalysis_1.getSavingsAnalysis)(userId, 180);
    if (savingsAnalysis) {
...
async function assignPersona(userId) {
    ...
    if (assignments.length > 1 && assignments[0].personaType === 'high_utilization') {
        ...
        const onlyWeakCriteria = !hasHighUtilization && (highUtilAssignment.criteriaMet.includes('interest_charges') ||
        ...
        const bestAlternative = alternatives.reduce((best, current) => current.confidence > best.confidence ? current : best, alternatives[0]);
