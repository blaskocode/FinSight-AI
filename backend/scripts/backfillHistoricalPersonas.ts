...
import { assignPersona } from '../personas/assignPersona';

/**
 * Check if persona already exists for a specific month
 */
async function personaExistsForMonth(userId: string, year: number, month: number): Promise<boolean> {
 ...
 */
export async function backfillUserPersonas(userId: string, monthsBack: number = 12): Promise<void> {
  ...
  for (let i = 0; i < monthsBack; i++) {
    ...
    if (exists) {
      ...
      const totalPersonas = await get<{ count: number }>(
 ...
 */
export async function backfillAllHistoricalPersonas(monthsBack: number = 12): Promise<void> {
