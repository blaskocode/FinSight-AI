...
exports.getPersonaHistory = getPersonaHistory;
exports.groupPersonaHistoryByMonth = groupPersonaHistoryByMonth;
 ...
 */
async function getPersonaHistory(userId, months = 12) {
    ...
    const history = await (0, db_1.all)(`SELECT persona_id, persona_type, assigned_at, signals, secondary_personas
     FROM personas
     WHERE user_id = ?
     ORDER BY assigned_at ASC`, [userId]);
    ...
    return filteredHistory.map(entry => {
        ...
        let parsedSecondaryPersonas = undefined;
        try {
 ...
 */
function groupPersonaHistoryByMonth(history) {
    if (history.length === 0) {
        return [];
    }
    const grouped = [];
