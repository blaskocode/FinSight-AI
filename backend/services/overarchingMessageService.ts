 ...
 */
export async function generateOverarchingMessage(userId: string): Promise<OverarchingMessage> {
  try {
    ...
    try {
      existingRecommendations = await getRecommendations(userId, 10);
    } catch (e) {
    ...
    if (personaType === 'high_utilization') {
      ...
      const nonDebtItems = actionableItems.filter(item => 
      ...
      const debtItems = actionableItems.filter(item => 
        item.title.toLowerCase().includes('debt') || 
        item.title.toLowerCase().includes('utilization') ||
        item.title.toLowerCase().includes('credit') ||
        item.title.toLowerCase().includes('interest') ||
    ...
    if (personaType === 'variable_income') {
      ...
      const emergencyItems = actionableItems.filter(item => 
    ...
    if (personaType === 'subscription_heavy') {
      ...
      const nonSubscriptionItems = actionableItems.filter(item => 
      ...
      const subscriptionItems = actionableItems.filter(item => 
        item.title.toLowerCase().includes('subscription') || 
        item.title.toLowerCase().includes('audit') ||
    ...
    if (personaType === 'savings_builder') {
      ...
      const savingsItems = actionableItems.filter(item => 
    ...
    if (personaType === 'lifestyle_creep') {
      ...
      const nonSavingsItems = actionableItems.filter(item => 
        !item.title.toLowerCase().includes('savings') && 
    ...
    if (actionableItems.length === 0) {
      ...
      if (personaType === 'subscription_heavy') {
        ...
        const fallbackItems: Record<string, ActionableItem> = {
          'high_utilization': {
            title: 'Monitor Your Credit Utilization',
            description: `Keep your credit card balances low and make payments on time. Aim to keep utilization below 30% to improve your credit score.`,
            priority: 'high'
    ...
    if (existingRecommendations.length > 0) {
      const highPriorityRecs = existingRecommendations.filter((r: any) => 
        r.rationale?.toLowerCase().includes('urgent') || 
    ...
    if (actionableItems.length === 0) {
      ...
      if (personaType === 'subscription_heavy') {
        ...
        const emergencyFallback: Record<string, ActionableItem> = {
          'high_utilization': {
            title: 'Monitor Your Credit Utilization',
            description: `Keep your credit card balances low and make payments on time. Aim to keep utilization below 30% to improve your credit score.`,
            priority: 'high'
      ...
      if (personaType === 'subscription_heavy') {
        const hasSubscriptionItem = actionableItems.some(item => 
          item.title.toLowerCase().includes('subscription') || 
          item.title.toLowerCase().includes('audit') ||
          item.title.toLowerCase().includes('cancel')
        );
        ...
        const hasDebtItem = actionableItems.some(item => 
          item.title.toLowerCase().includes('debt') || 
          item.title.toLowerCase().includes('utilization') ||
          item.title.toLowerCase().includes('credit') ||
          item.title.toLowerCase().includes('interest') ||
        ...
        const hasEmergencyItem = actionableItems.some(item => 
          item.title.toLowerCase().includes('emergency') || 
          item.title.toLowerCase().includes('fund') ||
          item.title.toLowerCase().includes('buffer')
        );
        ...
        const hasSavingsItem = actionableItems.some(item => 
          item.title.toLowerCase().includes('savings') || 
          item.title.toLowerCase().includes('emergency') ||
          item.title.toLowerCase().includes('fund') ||
          item.title.toLowerCase().includes('optimize') ||
    ...
    if (actionableItems.length === 0) {
      ...
      if (personaType === 'subscription_heavy') {
        ...
        const finalEmergencyFallback: Record<string, ActionableItem> = {
          'high_utilization': {
            title: 'Monitor Your Credit Utilization',
            description: `Keep your credit card balances low and make payments on time. Aim to keep utilization below 30% to improve your credit score.`,
            priority: 'high'
