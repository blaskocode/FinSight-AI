...
exports.UTILIZATION_THRESHOLDS = void 0;
exports.calculateUtilization = calculateUtilization;
exports.detectMinimumPaymentOnly = detectMinimumPaymentOnly;
exports.calculateInterestCharges = calculateInterestCharges;
exports.checkOverdueStatus = checkOverdueStatus;
exports.getCreditSignals = getCreditSignals;
const db_1 = require("../db/db");
// Utilization thresholds
exports.UTILIZATION_THRESHOLDS = {
 ...
 */
async function calculateUtilization(accountId) {
    // Get account with balances
    const account = await (0, db_1.get)('SELECT balances, type FROM accounts WHERE account_id = ?', [accountId]);
    if (!account) {
    ...
    let threshold;
    if (utilization >= exports.UTILIZATION_THRESHOLDS.CRITICAL * 100) {
        threshold = 'critical';
    }
    else if (utilization >= exports.UTILIZATION_THRESHOLDS.HIGH * 100) {
        threshold = 'high';
    }
    else if (utilization >= exports.UTILIZATION_THRESHOLDS.MEDIUM * 100) {
        // >=50% is high utilization per PRD
        threshold = 'high';
    }
    else if (utilization >= exports.UTILIZATION_THRESHOLDS.LOW * 100) {
        // >=30% but <50%: medium
        threshold = 'medium';
    }
    else {
    ...
    return {
        ...
        limit,
        threshold,
        isHighUtilization: utilization >= exports.UTILIZATION_THRESHOLDS.MEDIUM * 100
    };
}
/**
 ...
 */
async function detectMinimumPaymentOnly(accountId, windowDays = 90, asOfDate) {
    // Get liability information
    const liability = await (0, db_1.get)('SELECT minimum_payment_amount, last_payment_amount FROM liabilities WHERE account_id = ?', [accountId]);
    if (!liability) {
    ...
    const payments = await (0, db_1.all)(`SELECT ABS(amount) as amount, date 
     FROM transactions 
     WHERE account_id = ? 
       ...
       AND date <= ?
     ORDER BY date DESC
     LIMIT 3`, [accountId, cutoffDateStr, endDateStr]);
 ...
 */
async function calculateInterestCharges(accountId, windowDays = 90, asOfDate) {
    // Get liability APR
    const liability = await (0, db_1.get)('SELECT apr_percentage FROM liabilities WHERE account_id = ?', [accountId]);
    ...
    const account = await (0, db_1.get)('SELECT balances FROM accounts WHERE account_id = ?', [accountId]);
    if (!account) {
        return {
            totalCharges: 0,
            monthlyAverage: 0,
    ...
    const interestTransactions = await (0, db_1.all)(`SELECT ABS(amount) as amount 
     FROM transactions 
     WHERE account_id = ? 
       ...
       AND amount < 0
       AND date >= ?
       AND date <= ?`, [accountId, cutoffDateStr, endDateStr]);
 ...
 */
async function checkOverdueStatus(accountId, asOfDate) {
    const liability = await (0, db_1.get)('SELECT is_overdue, next_payment_due_date FROM liabilities WHERE account_id = ?', [accountId]);
    if (!liability) {
        return false;
    }
 ...
 */
async function getCreditSignals(accountId, windowDays = 90, asOfDate) {
