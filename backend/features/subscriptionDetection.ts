 ...
 */
export async function findRecurringMerchants(
  userId: string,
  ...
  for (const [merchantName, txs] of merchantGroups.entries()) {
    ...
    for (let i = 1; i < sortedTxs.length; i++) {
      const daysDiff = Math.round(
        (new Date(sortedTxs[i].date).getTime() - new Date(sortedTxs[i - 1].date).getTime()) / 
        (1000 * 60 * 60 * 24)
      );
 ...
 */
export function calculateRecurringCadence(
  intervals: number[],
  amounts: number[]
 ...
 */
function checkAmountVariance(amounts: number[]): number {
 ...
 */
export async function calculateMonthlyRecurringSpend(
  userId: string,
  windowDays: number = 90
): Promise<number> {
...
export async function calculateSubscriptionShare(
  ...
  const recurringMerchants = await findRecurringMerchants(userId, windowDays);
  const recurringSpend = recurringMerchants.reduce(
