...
exports.calculateMonthlyExpenses = calculateMonthlyExpenses;
exports.calculateNetSavingsInflow = calculateNetSavingsInflow;
exports.calculateSavingsGrowthRate = calculateSavingsGrowthRate;
exports.calculateEmergencyFundCoverage = calculateEmergencyFundCoverage;
exports.calculateSavingsRate = calculateSavingsRate;
exports.getSavingsAnalysis = getSavingsAnalysis;
 ...
 */
async function getSavingsAccounts(userId) {
    const accounts = await (0, db_1.all)(`SELECT account_id, type, balances
     FROM accounts
     WHERE user_id = ? 
       AND type IN ('savings', 'money_market', 'HSA')`, [userId]);
...
async function getUserAccountIds(userId) {
    const accounts = await (0, db_1.all)('SELECT account_id FROM accounts WHERE user_id = ?', [userId]);
 ...
 */
async function calculateMonthlyExpenses(userId, windowDays = 90) {
    ...
    const transactions = await (0, db_1.all)(`SELECT t.transaction_id, t.account_id, t.date, t.amount, t.merchant_name
     FROM transactions t
 ...
 */
async function calculateNetSavingsInflow(userId, windowDays = 90) {
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - windowDays);
    const cutoffDateStr = cutoffDate.toISOString().split('T')[0];
    ...
    if (savingsAccountIds.length > 0) {
        ...
        const savingsTransactions = await (0, db_1.all)(`SELECT transaction_id, account_id, date, amount, merchant_name
       FROM transactions
       WHERE account_id IN (${placeholders})
         AND date >= ?
         AND amount > 0`, [...savingsAccountIds, cutoffDateStr]);
    ...
    const checkingAccounts = await (0, db_1.all)(`SELECT account_id FROM accounts 
     WHERE user_id = ? AND type = 'checking'`, [userId]);
    if (checkingAccounts.length > 0) {
        const checkingAccountIds = checkingAccounts.map(acc => acc.account_id);
        const placeholders = checkingAccountIds.map(() => '?').join(',');
        const savingsTransfers = await (0, db_1.all)(`SELECT transaction_id, account_id, date, amount, merchant_name, personal_finance_category_detailed
       FROM transactions
       WHERE account_id IN (${placeholders})
         AND date >= ?
         AND amount < 0
        ...
        const transferAmount = savingsTransfers.reduce((sum, tx) => sum + Math.abs(tx.amount), 0);
 ...
 */
async function calculateSavingsGrowthRate(userId, windowDays = 90) {
    const savingsAccounts = await getSavingsAccounts(userId);
    if (savingsAccounts.length === 0) {
        return 0; // No savings accounts
    }
    // Get current total balance
    const currentBalance = savingsAccounts.reduce((sum, acc) => sum + (acc.balances.current || 0), 0);
 ...
 */
async function calculateEmergencyFundCoverage(userId) {
    // Get total savings balance
    const savingsAccounts = await getSavingsAccounts(userId);
    const totalSavings = savingsAccounts.reduce((sum, acc) => sum + (acc.balances.current || 0), 0);
    if (totalSavings === 0) {
        return 0;
    }
 ...
 */
async function calculateSavingsRate(userId, windowDays = 90) {
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - windowDays);
    const cutoffDateStr = cutoffDate.toISOString().split('T')[0];
    ...
    const incomeTransactions = await (0, db_1.all)(`SELECT transaction_id, account_id, date, amount, merchant_name
     FROM transactions
     WHERE account_id IN (${placeholders})
       AND date >= ?
       AND amount > 0`, [...checkingAccountIds, cutoffDateStr]);
 ...
 */
async function getSavingsAnalysis(userId, windowDays = 90) {
    const savingsAccounts = await getSavingsAccounts(userId);
    const totalSavingsBalance = savingsAccounts.reduce((sum, acc) => sum + (acc.balances.current || 0), 0);
