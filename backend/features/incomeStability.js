...
exports.detectPayrollACH = detectPayrollACH;
exports.detectPaymentFrequency = detectPaymentFrequency;
exports.calculatePayGapVariability = calculatePayGapVariability;
exports.calculateCashFlowBuffer = calculateCashFlowBuffer;
exports.getIncomeStabilityAnalysis = getIncomeStabilityAnalysis;
const db_1 = require("../db/db");
const savingsAnalysis_1 = require("./savingsAnalysis");
/**
 ...
 */
async function detectPayrollACH(userId) {
    ...
    const allTransactions = await (0, db_1.all)(`SELECT transaction_id, account_id, date, amount, merchant_name, payment_channel
     FROM transactions
    ...
    // Sort by date
    payrollTransactions.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
 ...
 */
function detectPaymentFrequency(incomeTransactions) {
    ...
    for (let i = 1; i < incomeTransactions.length; i++) {
        const daysDiff = Math.round((new Date(incomeTransactions[i].date).getTime() -
 ...
 */
function calculatePayGapVariability(incomeTransactions) {
    if (incomeTransactions.length < 2) {
 ...
 */
async function calculateCashFlowBuffer(userId) {
    // Get checking account balance
    const checkingAccounts = await (0, db_1.all)(`SELECT account_id, balances FROM accounts 
     WHERE user_id = ? AND type = 'checking'`, [userId]);
    if (checkingAccounts.length === 0) {
        return 0;
    }
    ...
    const monthlyExpenses = await (0, savingsAnalysis_1.calculateMonthlyExpenses)(userId, 180);
 ...
 */
function determineIncomeStability(medianPayGap, payGapVariability, cashFlowBuffer) {
 ...
 */
async function getIncomeStabilityAnalysis(userId) {
    ...
    const incomeStability = determineIncomeStability(medianPayGap, payGapVariability, cashFlowBuffer);
