 ...
 */
async function calculateHighUtilizationImpact(userId, recommendation, signals) {
 ...
 */
async function calculateVariableIncomeImpact(userId, recommendation, signals) {
    ...
    if (recommendation.type === 'budgeting_app') {
        ...
        if (payGapVariability > 10)
            return 100;
        if (payGapVariability > 5)
            return 75;
        return 50;
    }
    return 50; // Default impact
 ...
 */
async function calculateSubscriptionHeavyImpact(userId, recommendation, signals) {
    ...
    if (recommendation.type === 'subscription_manager') {
        ...
        if (monthlyRecurring > 200)
            return 100;
        if (monthlyRecurring > 100)
            return 75;
        if (monthlyRecurring > 50)
            return 50;
        return 25;
    }
    ...
    if (recommendation.type === 'education') {
        ...
        if (subscriptionShare > 20)
            return 100;
        if (subscriptionShare > 10)
            return 75;
        if (subscriptionShare > 5)
            return 50;
        return 25;
    }
    return 50; // Default impact
 ...
 */
async function calculateSavingsBuilderImpact(userId, recommendation, signals) {
    ...
    if (recommendation.type === 'education') {
        ...
        if (savingsRate < 10)
            return 100; // Low savings rate = high impact
        if (savingsRate < 15)
            return 75;
        if (savingsRate < 20)
            return 50;
 ...
 */
async function calculateLifestyleCreepImpact(userId, recommendation, signals) {
 ...
 */
async function calculateImpactScore(userId, personaType, recommendation, signals) {
    switch (personaType) {
 ...
 */
async function calculateUrgencyScore(userId, personaType, signals) {
    let urgency = 0;
 ...
 */
async function rankRecommendations(userId, recommendations, limit = 5) {
