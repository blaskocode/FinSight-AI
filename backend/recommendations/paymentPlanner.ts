...
// - Input validation: userId validated before use
// - Financial calculations: validated for accuracy
// - No user input directly in calculations (all data from database)
...
export interface Debt {
  ...
  balance: number;
  apr: number; // APR percentage (e.g., 18.5 for 18.5%)
...
export interface PaymentPlan {
  ...
  totalInterestSaved: number; // vs. minimum payments only
  payoffMonths: number;
 ...
 */
async function getUserDebts(userId: string): Promise<Debt[]> {
  const debts = await all<{
    ...
    `SELECT l.liability_id, l.account_id, l.type, l.apr_percentage, l.interest_rate, 
     ...
     ORDER BY l.last_statement_balance DESC`,
    [userId]
  );
  ...
  return debts.map(debt => ({
    ...
    type: debt.type as 'credit_card' | 'student_loan',
    ...
    accountName: (debt as any).account_name || undefined,
 ...
 */
export async function calculateAvailableCashFlow(userId: string): Promise<number> {
  ...
  const savingsAnalysis = await getSavingsAnalysis(userId);
 ...
 */
async function generateAvalanchePlan(
  userId: string,
  debts: Debt[],
  monthlySurplus: number
): Promise<PaymentPlan> {
 ...
 */
async function generateSnowballPlan(
  userId: string,
  debts: Debt[],
  monthlySurplus: number
): Promise<PaymentPlan> {
 ...
 */
export async function generatePaymentPlansComparison(userId: string): Promise<{
